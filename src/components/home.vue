<template>
  <div id="bobing_container">
    ...
    <audio src="http://others.cheertea.com/zhuangyuan.mp3" controls preload="auto" hidden="true"
           id="zhuangyuan"></audio>
    <!--博饼区-->
    <div class="main" :style="{backgroundImage: 'url(' + bobingDetail.moonStyle.styleMoonImage + ')'}">
      <div class="zhi">{{zhi}}</div>
      <div class="paihangbang" @click="paihangbang"></div>
      <div class="myXiangRoom" @click="myXiangRoom"></div>
      <div class="game_rule" @click="goRule"></div>
      <div class="about_us" @click="aboutUs"></div>
      <div class="tobuy" v-if="tobuyImage != null" @click="tobuy">
        <img :src="tobuyImage" alt="">
      </div>
      ...
      <div class="zhuanpan" @click="zhuanpan"></div>
      <div class="gainTime" @click="gainTime(0)"></div>
      <ul class="dicebox">
        <li v-for="(item,index) in diceArr" :style="{backgroundImage: 'url('+'http://images.cheertea.com/'+(index+1)+'.png)'}">
          {{item}}
        </li>
      </ul>
      <div class="begin_bobing" id="beginB" @click="beginBobing()"></div>
      <div class="shengyu">剩余{{leftTime}}次</div>
      <div class="huodongshijian">活动时间：{{beginTime}}-{{endTime}}</div>
    </div>
    <!--banner区-->
    <div class="vip_banner" @click="topHref" :style="{backgroundImage: 'url(' + bobingDetail.moonStyle.styleSpreadImage + ')'}"></div>
    <div class="gift_banner">
      <div class="git_top"></div>
      <div class="scroll-wrap" @click="lunboClick">
        <div class="inner-wrap clearfix">
          <div class="wrap-left">
            <img v-for="item in moonAwardList" :src="item.awardImage" alt="">
          </div>
    ...
    <div class="bottom_banner" @click="bottomHref" :style="{backgroundImage: 'url(' + bobingDetail.moonStyle.styleAdvertImage + ')'}" ></div>
    <div class="bottom_banner1" @click="bottomHref1" :style="{backgroundImage: 'url(' + bobingDetail.moonStyle.styleThemeImage + ')'}" ></div>
    <!--说明区-->
    ...
    <div class="mask">
      <div v-show="jieguoShow" class="jieguo_wrap">
        <div class="showmaskbox">
          ...
          <div v-show="meidejihui1" class="kuang2">
            ...
            <div class="shue">
              <p class="up">您的博饼机会已用完</p>
              <p class="down">分享和邀请好友加入能增加博饼次数哦！</p>
            </div>
          ...
          <div v-show="meidejihui2" class="kuang3">
            <div class="zhuhe">没有博饼机会啦</div>
            <div class="shue">
              <p class="up">消费一个订单满既定额度，增加{{dingdanjihui}}次博饼机会</p>
              <p class="down">依次类推，消费单数越多，博饼机会越多</p>
            </div>
            <div class="yaoqing" @click="gainTime(1)"></div>
          </div>
      ...
      <!--</div>-->
      <div v-show="zhuanpanShow" class="zhuanpan0_wrap">
        <p>消费一个订单满20元及以上，增加{{dingdanjihui}}次博饼机会</p>
        <!--<p>消费两个订单，增加{{dingdanjihui*2}}次博饼机会</p>-->
        <p>依次类推，消费单数越多，博饼机会越多</p>
        <div class="cha"  @click="clearT"></div>
        <div class="buy" @click="gainTime(1)"></div>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // document.getElementById('beginB').addEventListener('touchstart',
      // });
   ...
   $(function () {
     ...
     function Marquee1() {
      ...
      if (sessionStorage.getItem('bobingDetail')) {
        ...
        sessionStorage.buyNowHred = this.bobingDetail.moonStyle.buyNowHred
        ...
        this.moonAwardList = this.bobingDetail.moonAwardList
        ...
        this.tobuyImage = this.bobingDetail.moonStyle.buyNowImage
        // this.tobuyImage = null
        // $('.tobuy').css({
        //   background: 'url("' + this.bobingDetail.moonStyle.buyNowImage + '") 0 0 no-repeat',
    ...
    methods: {
          ...
          .then(data => {
            ...
            wx.ready(function () {
              let obj = {
                ...
                link: "http://" + _this.$urlstate.url + "/bobing/?#/yindao?roomId=" + _this.$route.query.roomId+"&memberId="+_this.memberId,
                ...
                success: function (res) {
                  ...
                  _this.$axios({
                    ...
                    // _this.leftTime += _this.moonShare;
      ...
      xuanhuozhuan() {
        if (this.winKindCode == 99) {
          ...
          //   }
          // }).then(data => {
          //   this.zhuanpanLeftTime = data.data.res_data.num;
          //   // if (this.zhuanpanLeftTime > 0) {
          //     this.$router.push('/zhuanpan')
          //   // }
          //   // else {
          //   //   $('.mask').show();
          ...
          //   })
          //   $('.fenx_wrap').css({
          //     "background": 'url("https://jymall.oss-cn-beijing.aliyuncs.com/mooncake/fenxT.png") 0 0 no-repeat',
          //     "backgroundSize": "100% 100%",
          //   })
          //   $('.showmaskbox').css({
          //     "background": "none"
      ...
      againGame() {
        ...
        if(this.moonCount%this.dengmiCondition == 0){
          var comId = (JSON.parse(sessionStorage.getItem('bobingInfo'))).comId;
          this.$axios({
            method: 'post',
            url: '/moon/anon/queryMoonRiddle',
            responseType: 'json',
            data: {
              comId: comId
            }
          }).then(data => {
            this.timu = data.data.res_data.moonRiddleQuestion.riddleQuestion;
            this.daan = data.data.res_data.moonRiddleAnswer;
            this.timuID = data.data.res_data.moonRiddleQuestion.questionId
          }).catch(err => {
            console.log(err)
          })
          $('.mask').show();
          // this.maskShow = true
          // this.tanchuang = 2;
          this.dengmiShow = true
          this.zhuanpanShow = false
          this.jieguoShow = false
          this.fenxShow = false
          this.jindu = true;
          this.datiqueding = true;
          $(function () {

            // alert('要猜灯谜啦')
            $('.jieguo_wrap').css({
              background: 'url("https://jymall.oss-cn-beijing.aliyuncs.com/mooncake/dengmi_bg.png") 0 0 no-repeat',
              backgroundSize: "100% 100%",
            })
            $('.dengmi_wrap').css({
              background: 'url("https://jymall.oss-cn-beijing.aliyuncs.com/mooncake/dengmi_bg.png") 0 0 no-repeat',
      ...
      tobuy(){
        window.location.href=sessionStorage.getItem('buyNowHred')
      },
      goRule() {
              ...
              // else {
                ...
                this.$axios({
                  ...
                  if (data.data.res_code == 0) {
                    ...
                    this.winOfficial = data.data.res_data.winOfficial;
...
<style scoped lang="scss">
  /*.backgroundHongImage {*/
  /*background: url("../assets/img/home_hongbaobg.png") 0 0 no-repeat !important;*/
  /*background-size: 100% 100% !important;*/
  /*}*/

  /*.backgroundBaoImage{*/
  /*background: url("../assets/img/home_yuanbaobg.png") 0 0 no-repeat !important;*/
  /*background-size: 100% 100% !important;*/
  /*}*/
  .active {
